...
class PowerBIExportPipeline:
    ...
    def __init__(self):
        ...
        # Load snapshot query path
        self._snapshot_query_dict = {
            "total_accruals_query":
                os.path.join(
                    self.config.base_dir, "queries", "total_accruals_query.dax"
                ),
            "total_executed_query":
                os.path.join(
                    self.config.base_dir, "queries", "total_executed_query.dax"
                ),
            "total_invoiced_query":
                os.path.join(
                    self.config.base_dir, "queries", "total_invoiced_query.dax"
                )
        }
    ...
    def _snapshot_data(self, connection, end_year: int, end_month: int) -> dict:
        """
        Execute snapshot queries for the 12-month period.

        Args:
            connection: PowerBI connection
            end_year: End year for the period
            end_month: End month for the period

        Returns:
            Dictionary containing the snapshot DataFrames
        """
        try:
            # Get date range and format query
            months = self.date_manager.calculate_12_month_range(
                end_year, end_month
            )

            query_params = {
                "year_end": months[-1][0],
                "month_end": months[-1][1],
                "day_end": 31,
                "year_start": months[0][0],
                "month_start": months[0][1],
                "day_start": 1  # Fixed typo
            }

            self.tracker.print_info(f"📸 Creating snapshot for period: "
                                    f"{months[0][0]}-{months[0][1]:02d} to "
                                    f"{months[-1][0]}-{months[-1][1]:02d}")

            # Initialize snapshot DataFrames
            snapshot_results = {}

            for query_name, query_path in self._snapshot_query_dict.items():
                try:
                    with open(query_path, 'r', encoding='utf-8') as f:
                        query_template = f.read()

                    # Format query with parameters
                    dax_query = query_template.format(**query_params)

                    # Execute query
                    datatable = self.query_executor.execute(connection, dax_query)

                    # Convert to DataFrame
                    df = self.data_processor.convert_to_dataframe(datatable)

                    # Store in results dictionary
                    snapshot_results[query_name] = df

                    self.tracker.print_success(f"✅ {query_name} executed: {len(df)} rows")

                except Exception as query_error:
                    self.tracker.print_error(f"❌ Failed to execute {query_name}: {str(query_error)}")
                    Logger.error(f"Snapshot query failed {query_name}: {str(query_error)}")
                    # Store empty DataFrame for failed queries
                    snapshot_results[query_name] = pd.DataFrame()

            # Extract individual DataFrames for backward compatibility
            df_accruals = snapshot_results.get("total_accruals_query", pd.DataFrame())
            df_executed = snapshot_results.get("total_executed_query", pd.DataFrame())
            df_invoiced = snapshot_results.get("total_invoiced_query", pd.DataFrame())

            # Log summary
            total_rows = sum(len(df) for df in snapshot_results.values())
            self.tracker.print_success(f"📊 Snapshot completed: {total_rows:,} total rows")

            # Export snapshot results
            self._export_snapshot_data(snapshot_results, end_year, end_month)

            return {
                "accruals": df_accruals,
                "executed": df_executed,
                "invoiced": df_invoiced,
                "all_results": snapshot_results
            }

        except Exception as e:
            error_msg = f"Snapshot failed for period {end_year}-{end_month:02d}: {str(e)}"
            self.tracker.print_error(f"❌ {error_msg}")
            Logger.error(error_msg)
            raise e

    def _export_snapshot_data(self, snapshot_results: dict, end_year: int, end_month: int):
        """Export snapshot data to files."""
        try:
            date_suffix = f"{end_year}-{end_month:02d}_snapshot"

            for query_name, df in snapshot_results.items():
                if len(df) > 0:
                    filename = f"{query_name}_{date_suffix}"

                    # Export to multiple formats
                    csv_path = self.csv_exporter.export(df, filename)
                    self.tracker.print_info(f"📄 Snapshot CSV: {csv_path}")

                    parquet_path = self.parquet_exporter.export(df, filename)
                    self.tracker.print_info(f"📦 Snapshot Parquet: {parquet_path}")

        except Exception as e:
            Logger.error(f"Failed to export snapshot data: {str(e)}")
